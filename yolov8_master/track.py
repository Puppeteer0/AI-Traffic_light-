...
def run(
        ...
        reid_weights=WEIGHTS / 'resnet50_msmt17_combineall_256x128_amsgrad_ep150_stp60_lr0.0015_b64_fb10_softmax_labelsmooth_flip_jitter.pt',  # model.pt path,
        ...
        project=ROOT / 'dist' / 'static' / 'media',  # save results to project/name
    ...
    kinds = {}
    time_line = []
    time_lines = [['time', 'classifier', 'count']]
    kp = ['person', 'car', 'bus', 'bicycle', 'motorcycle', 'truck', 'traffic light']
    kp_temp = ['person', 'car', 'bus', 'bicycle', 'motorcycle', 'truck', 'traffic light']  # label
        ...
        for i, det in enumerate(p):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for c in det[:, 5].unique():
                    ...
                    if (seen - 1) % 24 == 0 and names[int(c)] in kp:
                        kp_temp.remove(names[int(c)])
                        kinds[names[int(c)]] = n.item()
                        time_lines.append([(seen - 1) / 24, names[int(c)], n.item()])
                    s += f"{n} {names[int(c)]}{'s' * (n > 1)}, "  # add to string

                if (seen - 1) % 24 == 0:
                    for t in kp_temp:
                        time_lines.append([(seen - 1) / 24, t, 0])
                        kp_temp = ['person', 'car', 'bus', 'bicycle', 'motorcycle', 'truck', 'traffic light']
                    # time_line.append(kinds.copy())
    ...
    if dataset.mode != 'image':
        return [time_line,save_path, seen, time_lines]


def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--yolo-weights', nargs='+', type=Path, default=WEIGHTS / 'yolov8s-seg.pt', help='model.pt path(s)')
    parser.add_argument('--reid-weights', type=Path, default=WEIGHTS / 'resnet50_msmt17_combineall_256x128_amsgrad_ep150_stp60_lr0.0015_b64_fb10_softmax_labelsmooth_flip_jitter.pt')
    ...
    parser.add_argument('--project', default=ROOT / 'dist' / 'static'/'media', help='save results to project/name')
